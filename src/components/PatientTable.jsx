...
export const PatientTable = forwardRef(({
  ...
  // Component States initialized from static data
  ...
  // Reset page when filter props change
  useEffect(() => {
    setCurrentPage(1);
  ...
  const handleAddSubmit = (e) => {
    ...
    // Generate random patient ID PT-xxxx
  ...
  const filteredPatients = patients.filter((patient) => {
    ...
    if (dateFrom || dateTo) {
      if (patient.lastVisit && patient.lastVisit !== "New Patient" && patient.lastVisit !== "N/A") {
        ...
        matchesDate = false; // No valid last visit date to check
      }
    }
  ...
  const currentItems = isDashboard
    ? displayedPatients
    : displayedPatients.slice(indexOfFirstItem, indexOfLastItem);
  ...
  // Container styling matching dashboard / patients pages
  const containerClasses = isDashboard
    ...
    : "glass-card card-shadow rounded-xl overflow-hidden border border-outline-variant/30 w-full transition-all duration-300";
    ...
    <div className={containerClasses}>
      ...
      <AnimatePresence>
        {isViewModalOpen && selectedPatient && (
          <div className="modal-backdrop fixed inset-0 z-50 bg-inverse-surface/40 backdrop-blur-sm flex items-center justify-center p-4">
            <motion.div
        ...
        {isAddModalOpen && (
          <div className="modal-backdrop fixed inset-0 z-50 bg-inverse-surface/40 backdrop-blur-sm flex items-center justify-center p-4">
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1 }}
              exit={{ opacity: 0, scale: 0.95 }}
              className="glass-card card-shadow rounded-2xl max-w-xl w-full max-h-[90vh] overflow-y-auto border border-outline-variant/30 p-6 relative"
            >
              <button
              ...
              <h2 className="text-headline-sm font-headline-sm font-extrabold text-on-surface mb-4 flex items-center gap-2 select-none">
